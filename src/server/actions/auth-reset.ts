...
export async function requestPasswordReset(email: string) {
  try {
    ...
    const mailOptions = {
      from: `"Digital RWEE Security" <${process.env.SMTP_USER || 'noreply@desa.id'}>`,
      to: user.email,
      subject: 'Reset Password Akun Digital RWEE Anda',
          ...
          <div class="wrapper">
            <div class="main">
              <div class="header">
                ...
                <h1 class="title">Digital RWEE</h1>
                <div class="subtitle">Sistem Informasi Keuangan RW</div>
              </div>
              <div class="content">
                <p class="greeting">Halo ${user.name},</p>
                <p class="message">Kami menerima permintaan untuk mengatur ulang kata sandi <i>(reset password)</i> akun Anda di platform Digital RWEE.</p>
              ...
              <div class="footer">
                <p>&copy; ${new Date().getFullYear()} Digital RWEE Enterprise.<br>Email ini dikirim secara otomatis, mohon tidak membalas.</p>
              </div>
            </div>
          </div>
        </body>
